...
If you want deployments to run from a different branch name, update `.github/workflows/deploy-pages.yml` to match that branch before pushing, keep GitHub Pages set to **GitHub Actions**, and make sure that branch contains a valid `public/index.html`. You can also run the workflow manually with **workflow_dispatch**.
